...
void drive_to_sample(int angle){
  ...
  while((old_US_low >= US_low - error_margin) && (US_low >= distance_to_mountain) && (IR_1 != 1) && (IR_2 != 1)){ // while the distance does not increase and the distance to the mountainis not to close drive forwards
    old_US_low = US_low_sensor();
...
void mountain_function(){//function to drive to and around mountain
