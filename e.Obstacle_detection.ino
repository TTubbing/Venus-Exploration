...
/*void drive_to_mountain(int angle){
  ...
  do{
    ...
    IR_1 = IR_1_sensor();
    IR_2 = IR_2_sensor();
  }while(old_US_high >= US_high && old_US_low >= US_low && US_high >= distance_to_mountain && US_low >= distance_to_mountain && IR_1 != 1 && IR_2 != 1);
  Serial.print("done\n");
  stoppp();
}*/
void drive_to_mountain(int angle){
...
long US_low;
  ...
  int angle = 90; // find angle to drive parallel to mountain
  turn(angle);
  Usservo.write(180); // turn US sensor towards moountain
  forward();
  
  US_low = US_low_sensor();
  US_high = US_high_sensor();
  Serial.print("/n");
  Serial.print(US_high_sensor());
  Serial.print("/n");
  Serial.print(US_low_sensor());
  Serial.print("/n");
  Serial.print(out_of_sight);
  Serial.print("/n");
  while((US_high_sensor() <= out_of_sight) && (US_low_sensor() <= out_of_sight)){ // drive till you passed the mountain
